...
| `JERSEY_WEIGHTS` | `backend/weights/jersey_number_b0.pt` | EfficientNet jersey classifier checkpoint |
...
Place **`osnet_x1_0_soccernet.pth`** in `backend/weights/` (see [backend/weights/README.md](weights/README.md)). The analyzer uses OSNet embeddings to lock the target player by appearance when jersey OCR is ambiguous.
