    ...
    for i in range(40):
        ...
        if i < 20:
            ...
            target_samples.append(
                TargetFrameSample(frame=f, target_m=(x, y), target_visible=True)
            )
        else:
            ball_states.append(
                BallState(
...
def test_aggregates_passes_across_good_epochs():
    ...
    counts, _samples, best = run_events_multi(epochs, fps=30.0)
...
def test_off_pitch_epoch_contributes_nothing():
    ...
    counts, _samples, best = run_events_multi([good, bad], fps=30.0)
...
def test_no_lock_epoch_skipped():
    ...
    counts, _samples, best = run_events_multi([good, no_lock], fps=30.0)
    assert counts.Pass == 1
    assert best is good
...
def test_best_epoch_prefers_more_visible_frames():
    ...
    _counts, _samples, best = run_events_multi([short, long], fps=30.0)
    assert best is long
